...
export default function PopUpInforBuyer({ close, i, at, data }) {
  ...
  return (
    <div className="modal p-7" style={{ top: `calc(50% + ${at}px)` }}>
      ...
      <div className="m-5 p-5 border rounded-md border-green-sheen">
        <RowData titles={"Buyer id"} data={data[i]._id} />
        <RowData titles={"Address"} data={data[i].address} css={"mt-3"} />
        <RowData
          titles={"Post date"}
          data={formatDate(data[i].createdAt)}
          css={"mt-3"}
        />
        <RowData titles={"Buyer Name"} data={data[i].name} css={"mt-3"} />
        <RowData
          titles={"Is Blocked"}
